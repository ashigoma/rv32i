...
module mem (
    ...
    begin
      if (we2) begin
        if (ADR_START_2 <= adr2 && adr2 < ADR_START_2 + MEM_SIZE_2) begin
          mem2[adr_offset_2+3] <= wdata[31:24];
          mem2[adr_offset_2+2] <= wdata[23:16];
          mem2[adr_offset_2+1] <= wdata[15:8];
          mem2[adr_offset_2]   <= wdata[7:0];
        end
      end
    end
  end
  ...
  always_comb begin
    ...
    adr_offset_2 = adr2 - ADR_START_2;
    data2 = {mem2[adr_offset_2+3], mem2[adr_offset_2+2], mem2[adr_offset_2+1], mem2[adr_offset_2]};
  end
